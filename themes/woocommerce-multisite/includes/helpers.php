...
if ( ! function_exists( 'ecommerce_gem_fonts_url' ) ) {
	...
	function ecommerce_gem_fonts_url() {
		...
		if ( 'off' !== _x( 'on', 'Barlow font: on or off', 'woocommerce-multisite' ) ) {
			$fonts[] = 'Barlow:300,300i,400,400i,500,500i,600,600i,700,700i,800,800i';
		}

		/* translators: If there are characters in your language that are not supported by Playfair Display, translate this to 'off'. Do not translate into your own language. */
		if ( 'off' !== _x( 'on', 'Playfair Display font: on or off', 'woocommerce-multisite' ) ) {
...
if ( ! function_exists( 'ecommerce_gem_get_woocommerce_pages' ) ) :
	...
	function ecommerce_gem_get_woocommerce_pages() {
		...
		$check_pages = array(
			esc_html_x( 'Shop base', 'page setting', 'woocommerce-multisite' ) => array(
				'option'    => 'woocommerce_shop_page_id',
				'shortcode' => '',
			),
			esc_html_x( 'Cart', 'page setting', 'woocommerce-multisite' ) => array(
				'option'    => 'woocommerce_cart_page_id',
				'shortcode' => '[' . apply_filters( 'woocommerce_cart_shortcode_tag', 'woocommerce_cart' ) . ']',
			),
			esc_html_x( 'Checkout', 'page setting', 'woocommerce-multisite' ) => array(
				'option'    => 'woocommerce_checkout_page_id',
				'shortcode' => '[' . apply_filters( 'woocommerce_checkout_shortcode_tag', 'woocommerce_checkout' ) . ']',
			),
			esc_html_x( 'My account', 'page setting', 'woocommerce-multisite' ) => array(
...
if ( ! function_exists( 'ecommerce_gem_woocommerce_pages_status_message' ) ) :
	...
	function ecommerce_gem_woocommerce_pages_status_message() {
		...
		foreach ( $pages as $page ) {
			if ( true === $page['page_set'] ) {
				if ( true === $page['shortcode_required'] && true !== $page['shortcode_present'] ) {
					$output .= '<li>' . sprintf( esc_html__( '%1$s page does not contain %2$s shortcode.', 'woocommerce-multisite' ), $page['page_name'], $page['shortcode'] ) . '</li>';
				}
			} else {
				$output .= '<li>' . sprintf( esc_html__( '%s page is not set.', 'woocommerce-multisite' ), $page['page_name'] ) . '</li>';
			}
		}
