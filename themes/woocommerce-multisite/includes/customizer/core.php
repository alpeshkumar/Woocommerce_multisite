...
if ( ! function_exists( 'ecommerce_gem_get_default_theme_options' ) ) :
    ...
    function ecommerce_gem_get_default_theme_options() {
        ...
        $defaults['login_text']         = esc_html__( 'Login / Register', 'woocommerce-multisite' );
        ...
        $defaults['search_products_text']  = esc_html__( 'Search Products ...', 'woocommerce-multisite' );
        $defaults['select_category_text']  = esc_html__( 'Select Category', 'woocommerce-multisite' );

        // Layout.
        $defaults['enable_sticky_sidebar']  = true;
        $defaults['global_layout']          = 'right-sidebar';
        $defaults['excerpt_length']         = 40;
        $defaults['readmore_text']          = esc_html__( 'Read More', 'woocommerce-multisite' );
        ...
        $defaults['breadcrumb_text']    = esc_html__( 'Home', 'woocommerce-multisite' );

        // Slider.
        $defaults['slider_status']                  = false;
        $defaults['button_text_1']                  = esc_html__( 'Shop Now', 'woocommerce-multisite' );
        $defaults['button_text_2']                  = esc_html__( 'Shop Now', 'woocommerce-multisite' );
        $defaults['button_text_3']                  = esc_html__( 'Shop Now', 'woocommerce-multisite' );
        $defaults['button_text_4']                  = esc_html__( 'Shop Now', 'woocommerce-multisite' );
        $defaults['button_text_5']                  = esc_html__( 'Shop Now', 'woocommerce-multisite' );
