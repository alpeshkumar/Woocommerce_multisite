...
if ( ! class_exists( 'eCommerce_Gem_About' ) ) {
	...
	class eCommerce_Gem_About {
		...
		public function setup_config() {
			...
			$this->notification  = isset( $this->config['notification'] ) ? $this->config['notification'] : ( '<p>' . sprintf( esc_html__( 'Welcome! Thank you for choosing %1$s! To fully take advantage of the best our theme can offer please make sure you visit our %2$swelcome page%3$s.', 'woocommerce-multisite' ), $this->theme_name, '<a href="' . esc_url( admin_url( 'themes.php?page=' . $this->page_slug ) ) . '">', '</a>' ) . '</p><p><a href="' . esc_url( admin_url( 'themes.php?page=' . $this->page_slug ) ) . '" class="button button-primary" style="text-decoration: none;">' . sprintf( esc_html__( 'Get started with %s', 'woocommerce-multisite' ), $this->theme_name ) . '</a></p>' );
		}
		...
		public function load_assets( $hook ) {
			...
			if ( 'appearance_page_' . $this->page_slug === $hook ) {
				...
				wp_enqueue_style( 'woocommerce-multisite-about', get_template_directory_uri() . '/includes/theme-info/css/about.css', array(), '2.1.3' );
				wp_enqueue_script( 'woocommerce-multisite-about', get_template_directory_uri() . '/includes/theme-info/js/about.js', array( 'jquery' ), '2.1.3' );
				$js_vars = array(
					'ajaxurl' => esc_url( admin_url( 'admin-ajax.php' ) ),
				);
				wp_localize_script( 'woocommerce-multisite-about', 'eCommerceGemAboutObject', $js_vars );
			}
		}
						...
						if ( ! empty( $action_value['plugin_slug'] ) ) {
							...
							switch ( $active['needs'] ) {
								...
								case 'install':
									...
									$label = esc_html__( 'Install', 'woocommerce-multisite' );
									break;
								case 'activate':
									$class = 'activate-now button button-primary';
									$label = esc_html__( 'Activate', 'woocommerce-multisite' );
									break;
								case 'deactivate':
									$class = 'deactivate-now button';
									$label = esc_html__( 'Deactivate', 'woocommerce-multisite' );
									break;
							}
							?>
		...
		public function upgrade_to_pro() {
			...
			if ( isset( $upgrade_to_pro['button_link'] ) && ! empty( $upgrade_to_pro['button_link'] ) ) {
				$button_text = esc_html__( 'Upgrade to Pro', 'woocommerce-multisite' );
		...
		public function useful_plugins() {
			...
			if ( isset( $tgmpa ) ) {
				...
				if ( true === $complete ) {
					echo '<div class="all-active"><span class="dashicons dashicons-yes"></span>&nbsp;' . esc_html__( 'Great, all recommended plugins are intalled and active.', 'woocommerce-multisite' ) . '</div>';
				}
				...
				if ( true !== $complete ) {
					echo '<div class="button-wrap"><a href="' . esc_url( $url ) . '" class="button button-primary">' . esc_html__( 'Manage Plugins', 'woocommerce-multisite' ) . '</a></div>';
				}
		...
		public function free_pro() {
		    ...
		    if ( ! empty( $free_pro ) ) {
		        ...
		        if ( ! empty( $free_pro ) && is_array( $free_pro ) ) {
		            ...
		            echo '<tr>';
		            echo '<th></th>';
		            echo '<th>' . esc_html__( 'Woocommerce Multisite','woocommerce-multisite' ) . '</th>';
		            echo '<th>' . esc_html__( 'Woocommerce Multisite Plus','woocommerce-multisite' ) . '</th>';
		            echo '</tr>';
		            echo '</thead>';
		            echo '<tbody>';
		            ...
		            echo '<td></td>';
		            echo '<td colspan="2"><a href="https://www.prodesigns.com/wordpress-themes/downloads/ecommerce-gem-plus/" target="_blank" class="button button-primary button-hero">Woocommerce Multisite Plus</a></td>';
		            echo '</tr>';
